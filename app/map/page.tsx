...
export default function MapPage() {
  ...
  const factionColor = (id: number | null) =>
    id === null ? INK : id === 0 ? CORAL : id === 1 ? SEA : GOLD;
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <header className="mb-8">
          ...
          <p className="mt-2 max-w-2xl text-sm opacity-75">
            ...
            Nothing here is inferred — both axes come straight from classified stances across
            every forum and labs post.
          </p>
        </header>
          ...
          <>
            ...
            >
              ...
              <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 540 }}>
                {/* Quadrant shading around the hive average */}
                <rect
                  x={px(scales.xMin)}
                  y={py(scales.yMax)}
                  width={px(data.hive.avgConviction) - px(scales.xMin)}
                  height={py(data.hive.avgWinRate) - py(scales.yMax)}
                  fill={SEA}
                  opacity={0.04}
                />
                <rect
                  x={px(data.hive.avgConviction)}
                  y={py(data.hive.avgWinRate)}
                  width={px(scales.xMax) - px(data.hive.avgConviction)}
                  height={py(scales.yMin) - py(data.hive.avgWinRate)}
                  fill={CORAL}
                  opacity={0.04}
                />
                ...
                {/* Average crosshairs */}
                <line
                  x1={px(data.hive.avgConviction)}
                  y1={PAD.top}
                  x2={px(data.hive.avgConviction)}
                  y2={H - PAD.bottom}
                  stroke={INK}
                  strokeOpacity={0.18}
                  strokeDasharray="4 4"
                ...
                <line
                  x1={PAD.left}
                  y1={py(data.hive.avgWinRate)}
                  x2={W - PAD.right}
                  y2={py(data.hive.avgWinRate)}
                  stroke={INK}
                  strokeOpacity={0.18}
                  strokeDasharray="4 4"
                />
                ...
                <text
                  ...
                  letterSpacing="1.5"
                >
                  CONVICTION — SHARE OF HARD YES / NO STANCES →
                </text>
                <text
                ...
                {/* Faction links — drawn under the dots */}
                {data.factions.map((f) =>
                  f.members.flatMap((a, i) =>
                    f.members.slice(i + 1).map((b) => {
                      const la = plotted.find((l) => l.wallet === a);
                      const lb = plotted.find((l) => l.wallet === b);
                  ...
                  return (
                    <g key={l.wallet}>
                      <circle
                        ...
                        stroke={isSel ? INK : l.faction !== null ? factionColor(l.faction) : "#fff"}
                        strokeWidth={isSel ? 2.5 : l.faction !== null ? 2 : 1}
              ...
              <div className="mt-2 flex flex-wrap gap-4 font-mono text-[10px] uppercase tracking-widest opacity-50">
                ...
                <span>dot color = specimen hue</span>
                <span>ring + line = faction</span>
            ...
            {data.factions.length > 0 && (
              ...
              >
                ...
                <p className="mt-1 mb-4 text-sm opacity-70">
                  ...
                  posts. Both are small and loosely bound — worth watching, not worth alarm.
                </p>
