...
// is instant. Generating one board = N in-character responses (batched Gemini
// calls — several larvae per request) + one clustering call that groups those
// responses into ranked board answers.
//
...
function cleanSurveyAnswer(raw: string): string | null {
  const answer = raw
    .trim()
    .replace(/^["'`]+|["'`.,!]+$/g, "")
    .replace(/\s+/g, " ")
  ...
  return (kept.length > 0 ? kept : words).join(" ").slice(0, 60);
}
...
const SURVEY_BATCH_SIZE = 8;
const SURVEY_BATCH_CONCURRENCY = 4;

const SURVEY_BATCH_SYSTEM = `You are simulating several larvae — personal AI governance agents in the $CLAWD ecosystem — answering the SAME survey question, each in their own voice.

For EVERY larva listed, invent ONE short answer in that larva's personality.

Rules for each answer:
- Usually 1-3 words (titles and proper nouns are fine). Not a sentence. Not an explanation.
- MUST match the required category. Off-category answers are invalid.
- Name the thing itself — no job fluff, no "definitely/probably/lowkey".
- Do NOT answer with abstract crypto jargon unless the question literally asks for that.
- Unusual personality picks are fine; wrong category is not.
- Different larvae should often give different answers.

Respond with ONLY a JSON array, no markdown:
[{"i":1,"answer":"..."},{"i":2,"answer":"..."}, ...]
Include exactly one object per larva, using their index numbers.`;

async function surveyBatch(
  personas: SurveyPersona[],
  question: string
  ...
  const roster = personas
    .map(
      (p, i) =>
        `${i + 1}. name="${p.name}" | tone=${p.tone} | values=${p.values} | quirks=${p.quirks} | about=${p.summary.slice(0, 160)}`
    )
    .join("\n");
  ...
  try {
    const raw = await haiku(
      ...
      Math.min(80 + personas.length * 40, 600),
      0.85
    );
    const clean = raw.replace(/```json|```/g, "").trim();
    const start = clean.indexOf("[");
    ...
    for (const row of parsed) {
      ...
      const answerRaw = String((row as { answer?: unknown }).answer || "");
      if (!Number.isInteger(i) || i < 1 || i > personas.length || seen.has(i)) continue;
      const cleaned = cleanSurveyAnswer(answerRaw);
      if (!cleaned) continue;
      seen.add(i);
      const p = personas[i - 1];
      out.push({ wallet: p.wallet, name: p.name, answer: cleaned });
    }
    return out;
  } catch {
    return [];
...
/** Load profiles then survey in Gemini batches (Hobby 60s needs this). */
async function surveyRespondents(
  wallets: string[],
  question: string
): Promise<SurveyResponse[]> {
  const profiles = await mapWithConcurrency(wallets, 20, (w) => getProfile(w));
  const personas: SurveyPersona[] = [];
  ...
  const batches: SurveyPersona[][] = [];
  for (let i = 0; i < personas.length; i += SURVEY_BATCH_SIZE) {
    batches.push(personas.slice(i, i + SURVEY_BATCH_SIZE));
  }

  const parts = await mapWithConcurrency(batches, SURVEY_BATCH_CONCURRENCY, (batch) =>
    surveyBatch(batch, question)
  );
  return parts.flat();
}
...
const RESPONDENT_COUNT = Number(process.env.SURVEY_RESPONDENTS || 32);

/**
 * Build one complete board: survey N larvae, cluster their answers, rank them.
 * Survey is batched (~8 larvae per Gemini call) so Hobby 60s can finish.
 */
export async function buildBoard(questionId: string): Promise<SurveyBoard | null> {
  ...
  const responses = await surveyRespondents(
    respondents.map((r) => r.wallet),
    q.text
  );
